...
/*   Updated: 2026/04/29 15:08:43 by miouali          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int ac, char **av)
{
	...
	global->tab = malloc(sizeof(t_tab_of_thread) * (global->number_of_philo + 1));
	if (!global->tab)
	{
		exit_philo(global);
		return (1);
	...
	global->fork = malloc(sizeof(pthread_mutex_t) * (global->number_of_philo + 1));
	if (!global->fork)
	{
		exit_philo(global);
		return (1);
